...
if facesCnt > 0:
    ...
    for face in faces:
        ...
        # 원본이미지에 이모티콘 처리한 얼굴 이미지 붙이기
        image[y-h:y, x:x + w] = face_image

        print("x : ", x," y : ", y, " w : ", w, " h : ", h)

    # 이모티콘 처리된 이미지 파일 생성하기
    cv2.imwrite("result/redvelvet_crown.jpg", image)

    # 이모티콘 처리된 이미지 보여주기
    cv2.imshow("emoticon_image", cv2.imread("result/redvelvet_crown.jpg", cv2.IMREAD_COLOR))
