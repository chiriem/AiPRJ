...
# 분석하기 위한 이미지 불러오기
image = cv2.imread("image/redvelvet.jpg", cv2.IMREAD_COLOR)
...
# 얼굴을 변경할 이모티콘 이미지
emoticon_image = cv2.imread("image/3.png", cv2.IMREAD_COLOR)
...
if facesCnt > 0:
    ...
    cv2.imwrite("result/redvelvet_emoticon.jpg", image)

    # 이모티콘 처리된 이미지 보여주기
    cv2.imshow("emoticon_image", cv2.imread("result/redvelvet_emoticon.jpg", cv2.IMREAD_COLOR))
