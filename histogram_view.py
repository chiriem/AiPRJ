...
# OpenCV 색상을 일반적으로 부르는 RGB 순서가 아닌 BGR 순서로 사용
color = ('b', 'g', 'r')
...
for i, col in enumerate(color):
    ...
    # 두 번째 파라미터(Channel) : 컬러이미지(BGR)이면, 배열 값 1개로 정의
    # 세 번째 파라미터(Mask) : 분석할 영역의 형태인 mask
    # 네 번째 파라미터(histSize) : 히스토그램의 hist 크기, 예 : 64면 256/64 = 4 =>  픽셀 4개를 1개의 픽셀로 합쳐 연산
    # 다섯 번째 파라미터(범위) : 컬러이미지(BGR)이면 8 ~ 256까지 배열
